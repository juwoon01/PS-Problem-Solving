...
풀이
  1. 입력이 들어올 때 마다 왼쪽의 탑들과 비교 ->Time Out
  2. 6 9 5 3 7 10 3 2 를 봤을때 전의 모든 값보다 큰 값이 들어왔을때 뒤의 값들은 필요 없어진다.
  3. 비교하여 현재 값보다 작은 값을은 빼버리고
   3.1 현재값보다 큰 값을 만나거나 없으면 stack에 push한다.
  4. 또 Time Out이 나서 입출력을 c로 바꾸니 통과하였다.
     앞으로 시간제한이 널널한게 아니면 c 입출력을 사용해야겠다.
*/
